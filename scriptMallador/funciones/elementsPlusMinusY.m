function [elementsMinusNumber,elementsPlusNumber] = elementsPlusMinusY(nodesIndex,nodes,elements,elementsFisuIndex)
...
for iEle = 1:size(elementsFisu,1)
    ...
    b=~a;
    caraPlus_yCoord=nodes(elementsFisu(iEle,a),2);
    caraMinus_yCoord=nodes(elementsFisu(iEle,b),2);
    for iNodFace = 1:length(caraPlus_yCoord)
        if caraPlus_yCoord(iNodFace)>caraMinus_yCoord(iNodFace)
