...
assert(sum(cara.norte)>0 & sum(cara.sur)>0 & sum(cara.este)>0 & sum(cara.oeste)>0 & sum(cara.superior)>0 & sum(cara.inferior)>0); %para atrapar errores en la malla: por si no detecta nodos en alguna cara
end
