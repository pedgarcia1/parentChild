function [elementArray,nodesPositionArray]=trussFrameMeshGeneratorCasing(structuralMembersArray,structuralJointsArray);
...
for iStructuralJoints=1:max(max(structuralMembersArray.nodes(:,1:2))) %Only nodes are considered not auxiliar points
    nNodes=nNodes+1;
    n(nNodes,:)=structuralJointsArray(iStructuralJoints,:);
end
