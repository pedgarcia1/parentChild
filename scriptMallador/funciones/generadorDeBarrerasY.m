...
%%% NODOS BARRERA %%% 
nodosBarrera = find(abs(nodes(:,2)-posBarrera1)<tol);
nodosBarreraLogical = abs(nodes(:,2)-posBarrera1)<tol;
nodosPlusLogical = nodes(:,2)>(posBarrera1 - tol);
...
[elementsFisu.minus,elementsFisu.plus] = elementsPlusMinusY(nodosBarrera,nodes,elements,elementsFisu.index);
...
for iEle = 1:size(elementsDosNodos,1)
    ...
    if sum(nodes(nodesEle,2)>(posBarrera1 + tol)) > 4
...
nodesNew(nodosPlus,2) = nodesNew(nodosPlus,2) + espesorBarrera1; 
