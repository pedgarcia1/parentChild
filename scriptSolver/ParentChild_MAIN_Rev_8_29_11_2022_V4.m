...
direccionGuardado = 'C:\Users\pgarcia\Documents\Parent-Child Rev.8 29-11-2022\Resultados de corridas\ParentChild_Pedro_01_06_23\'; % Direccion donde se guarda la informacion.
nombreCorrida     = 'ParentChild_Pedro'; % Nombre de la corrida. La corrida se guarda en la carpeta "Resultado de corridas" en una subcarpeta con este nombre.

cargaDatos     = 'load'; % Forma en la que se cargan las propiedades de entrada. "load" "test" "default" "change".
archivoLectura = 'ParentChild_Pedro_1.txt'; % Nombre del archivo con las propiedades de entrada. 
...
propiedadesRestart = 'pc_reducido_itime_16.mat';
...
% meshInfo.cohesivos.elements = meshInfo.cohesivos.elements(2:end,:);
% meshInfo.cohesivos.related8Nodes = meshInfo.cohesivos.related8Nodes(2:end,:);
% meshInfo.cohesivos.type = meshInfo.cohesivos.type(2:end,:);
% meshInfo.cohesivos.name = meshInfo.cohesivos.name(2:end);
% meshInfo.cohesivos.relatedEB = meshInfo.cohesivos.relatedEB(2:end,:);
% meshInfo.cohesivos.nodesEle = meshInfo.cohesivos.nodesEle(:,:,2:end);
% meshInfo.cohesivos.T = meshInfo.cohesivos.T(:,:,2:end);
% meshInfo.nCohesivos = meshInfo.nCohesivos -1 ; 
meshInfo.constraintsRelations = meshInfo.constraintsRelations(1:288,1:5);
% meshInfo.elementsFluidos.elements = meshInfo.elementsFluidos.elements(2:end,:);
% meshInfo.elementsFluidos.relatedEB = meshInfo.elementsFluidos.relatedEB(2:end,:);
% meshInfo.elementsFluidos.nodesEle = meshInfo.elementsFluidos.nodesEle(:,:,2:end);
% meshInfo.elementsFluidos.T = meshInfo.elementsFluidos.T(:,:,2:end);
% % meshInfo.elementsFluidos.activos = meshInfo.elementsFluidos.activos(2:end);
% % meshInfo.elementsFluidos.activos(2:end) = meshInfo.elementsFluidos.activosTimes(2:end);
% meshInfo.nFluidos = meshInfo.nFluidos -1;
