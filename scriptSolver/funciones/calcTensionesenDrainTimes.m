...
iTimeEspecifico = temporalProperties.drainTimes; % OK ESTO??? %sum(tiempo <= tiempoTensiones) + temporalProperties.drainTimes;
...
tensionHidroDrainTimes=zeros(paramDiscEle.nel,1);

for iele=1:paramDiscEle.nel
    tensionHidroDrainTimes(iele)=tensionEfectivaNodal(iele,1)+tensionEfectivaNodal(iele,2)+tensionEfectivaNodal(iele,3);
end
