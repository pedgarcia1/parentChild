function [ elementalMatrix ] = getElementalMatrix(pGaussParam,paramDiscEle,nodesEle,propertie,key)

if strcmpi(key,'K')
    constitutivas = propertie;
...
elseif strcmpi(key,'C')
    Ce = sparse(paramDiscEle.nDofNod*paramDiscEle.nNodEle,paramDiscEle.nNodEle); 
    Biot = propertie;
    for ipg = 1:pGaussParam.npg
        % Punto de Gauss
        ksi = pGaussParam.upg(ipg,1);
...
%           
        ...
        Ce = Ce + B'*Biot*pGaussParam.wpg(ipg)*N*det(jac);
