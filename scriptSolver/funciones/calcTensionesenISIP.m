...
Elem=find(max(meshInfo.elements'== bombaProperties.nodoBomba(1)),1);
% indiceTiempo = temporalProperties.drainTimes+1:iTime;
% tiempo = cumsum(temporalProperties.deltaTs(indiceTiempo));
iTimeEspecifico = iTime-1; %find((tiempo>temporalProperties.tInicioISIP)); %sum(tiempo <= tiempoTensiones) + temporalProperties.drainTimes;
