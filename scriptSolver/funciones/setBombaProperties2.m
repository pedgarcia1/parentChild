function [ bombaProperties ] = setBombaProperties2(meshInfo,key,key2,varargin )
...
elseif strcmpi(key2,'change')
    ...
    for i=1:bombaProperties.nBombas
        possibleNode                 = getNodesInPos(0.001,meshInfo,posNodoBomba(i,:));
...
disp(['--nProduccion: '     num2str(bombaProperties.nProduccion)]);
disp(['--nodoProduccion: '     num2str(bombaProperties.nodoProduccion)]);
format loose
...
bombaProperties.Qbombas = [0*ones(bombaProperties.nBombas,2),                    bombaProperties.Qbombas, zeros(bombaProperties.nBombas,3),                    bombaProperties.QProduccion*ones(bombaProperties.nBombas,2)];
bombaProperties.tbombas = [-1*ones(bombaProperties.nBombas,1), tInicioInyeccion, bombaProperties.tbombas, tFinInyeccion, tInicioISIP+tiempoISIP/4, tFinalISIP, tInicioProduccion,           tFinalProduccion];
