...
%  corresponding position and vector component. The string title specifies
...
%  tetrahedrons. 
%  
%  vtkwrite(filename,'polydata','hexahedron',x,y,z,hexa) exports a list
%  of hexahedrons where x,y,z are the coordinates of the points
%  and hexa is an m*8 matrix whose rows denote the points of individual
%  tetrahedrons. 
%  
...
%                 fwrite(fid, reshape(varargin{sidx(ii)+2},1,n_elements),'float','b');
    ...
    case 'CELL'
        ...
        vidx = find(strcmpi(varargin,'VECTORS'));
        sidx = find(strcmpi(varargin,'SCALARS'));
        cidx = find(strcmpi(varargin,'CELLDATASCALARS'));
        ...
        if cidx~=0
            for ii = 1:length(cidx)
                if first_print_flag == false
                    fprintf(fid, ['\nCELL_DATA ' num2str(nhexa)]);
                end
                title = varargin{cidx(ii)+1};
                fprintf(fid, ['\nSCALARS ', title,' float\n']);
                fprintf(fid, 'LOOKUP_TABLE default\n');
                if ~binaryflag
                    spec = [repmat(['%0.', precision, 'f '], 1, 9), '\n'];
                    fprintf(fid, spec, varargin{ cidx(ii) + 2});
                else
                    fwrite(fid, varargin{ cidx(ii) + 2}, 'float', 'b');
                end
                %                 fwrite(fid, reshape(varargin{sidx(ii)+2},1,n_elements),'float','b');
