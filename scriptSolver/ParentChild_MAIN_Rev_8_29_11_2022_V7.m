...
archivoLectura = 'ParentChild_Pedro_5.txt'; % Nombre del archivo con las propiedades de entrada. 
...
%             errorRelCohesivos{iTime}(1,nIter) = norm(dN - dNPrevITER_Error);
        ...
        if nIter > algorithmProperties.nIterDiv
            ...
            deltaT          = deltaT/2;
            if deltaT <= 5e-3 % OJO CAMBIAR DE NUEVO A 1e-9
                warning 'Error: deltaT <= 1e-9. Posible divergencia eterna. Revisar parametros.';
                warning 'Reduccion de Qbomba a 0.1BPM y deltaT a 0.1s para converger ISIP'.
                deltaT = 1;
                temporalProperties.tFinalFrac(iFractura) = temporalProperties.tFinalFrac(iFractura) + 1;
                %                 algorithmProperties.elapsedTime < temporalProperties.tInicioISIP(iFractura) % Estamos fracturando y antes del ISIP.
                temporalProperties.tInicioISIP(iFractura) = temporalProperties.tInicioISIP(iFractura) + 1;
                Qbomba = 0.1*0.0026497882488*1e9; % 0.1 BPM a mm3/s
                Q = sparse(bombaProperties.nodoBomba(iFractura),1,Qbomba,paramDiscEle.nDofTot_P,1); % Solo caudal en la fractura actual.
            end
